...
const Pagination = ({ total, current, onPageChange, siblingCount = 1, boundaryCount = 1 }) => {
      ...
      <div className="flex justify-center space-x-1">
        ...
        {range(1, total).map((page) => (
          <button
            ...
            className={`px-2 py-1 border rounded ${current === page ? 'bg-blue-500 text-white' : ''}`}
          >
            {page}
          </button>
        ))}
  ...
  return (
    <div className="flex justify-center space-x-1" style={{background: 'white', margin: '10px 10px', padding: '10px'}}>
      <button
        onClick={() => onPageChange(current - 1)}
        disabled={current === 1}
        className="px-2 py-1 border rounded disabled:opacity-50"
